...
// not a lot to say here--nice routes!
app.use(cors());
...
app.get('/search/:id', async(req, res) => {
  try {
    const data = await request.get(`https://api.giphy.com/v1/gifs/${req.params.id}?api_key=${process.env.GIPHY_KEY}`);
...
//expects a list of ids. For example: '/giphy-favorites/UqwsxjkQjsIjcTjVx7,ZdgEG4nNI4KJL79YKDx'
app.get('/api/giphy-favorites/:ids', async(req, res) => {
  try {
    const data = await request.get(`https://api.giphy.com/v1/gifs?api_key=${process.env.GIPHY_KEY}&ids=${req.params.ids}`);
